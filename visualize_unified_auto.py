...
class UnifiedAutoVisualizer:
    ...
    def plot_attribute_inference(self):
        ...
        # 使用GridSpec获得更好的布局控制，为底部图例留出足够空间
        from matplotlib.gridspec import GridSpec
        gs = GridSpec(2, 3, figure=fig, hspace=0.4, wspace=0.35, 
                     bottom=0.15, top=0.96, left=0.07, right=0.97)
        
        # 子图1: 不同方法在各隐藏比例下的表现
        ax1 = fig.add_subplot(gs[0, 0])
        methods = sorted(set(item['method'] for item in data))
        # 检查是否有label_type字段
        has_label_type = any('label_type' in item for item in data)
        
        if has_label_type:
            # 分别显示Circles和Feat
            label_types = sorted(set(item.get('label_type', 'Unknown') for item in data))
            width = 0.15
            ...
            for label_type in label_types:
                for i, method in enumerate(methods):
                    accs = [next((item['accuracy'] * 100 for item in data 
                                 if item['method'] == method and item['hide_ratio'] == ratio 
                                 and item.get('label_type') == label_type), 0)
                           for ratio in hide_ratios]
                    label = f"{method} ({label_type})" if len(label_types) > 1 else method
                    ax1.bar(x + (color_idx * width), accs, width, label=label, alpha=0.8)
                    color_idx += 1
        else:
            # 原有逻辑
            width = 0.25
        ...
        if has_label_type:
            ...
            for label_type in label_types:
                for method in methods:
                    ...
                    if method_data:
                        method_data.sort(key=lambda x: x['hide_ratio'])
                        accs = [item['accuracy'] * 100 for item in method_data]
                        method_hide_ratios = [item['hide_ratio'] * 100 for item in method_data]
                        label = f"{method} ({label_type})" if len(label_types) > 1 else method
                        ax2.plot(method_hide_ratios, accs, marker='o', 
        ...
        if has_label_type:
            ...
            for label_type in label_types:
                for method in methods:
                    method_data = [item['accuracy'] * 100 for item in data 
                                 if item['method'] == method and item.get('label_type') == label_type]
                    if method_data:
                        all_method_labels.append(f"{method} ({label_type})")
                        all_avg_accs.append(np.mean(method_data))
            
            colors = plt.cm.Set3(np.linspace(0, 1, len(all_method_labels)))
            ax3.barh(all_method_labels, all_avg_accs, color=colors, alpha=0.8)
        else:
            # 原有逻辑
            avg_accs = []
            for method in methods:
            ...
            if has_label_type:
                ...
                for label_type in label_types:
                    for method in methods:
                        method_data = [item for item in data 
                                     if item['method'] == method and item.get('label_type') == label_type 
                                     and 'f1_macro' in item]
                        if method_data:
                            method_data.sort(key=lambda x: x['hide_ratio'])
                            f1_scores = [item.get('f1_macro', 0) * 100 for item in method_data]
                            hide_ratios_with_f1 = [item['hide_ratio'] * 100 for item in method_data]
                            label = f"{method} ({label_type})" if len(label_types) > 1 else method
        ...
        # 检查是否有label_type字段（Circles vs Feat）
        has_label_type = any('label_type' in item for item in data)
            ...
            for label_type in label_types:
                for method in methods:
                    ...
                    if method_data:
                        # 按hide_ratio排序
                        method_data.sort(key=lambda x: x['hide_ratio'])
                        correct_counts = [item.get('correct', 0) for item in method_data]
                        total_counts = [item.get('total', 1) for item in method_data]
                        ratios = [c / t * 100 for c, t in zip(correct_counts, total_counts)]
                        method_hide_ratios = [item['hide_ratio'] * 100 for item in method_data]
                        
                        label_suffix = f" ({label_type})" if len(label_types) > 1 else ""
                        ax5.plot(method_hide_ratios, ratios, marker='^', 
                                label=f"{method}{label_suffix}", linewidth=2, markersize=8)
        else:
            # 原有逻辑（没有label_type）
            for method in methods:
                method_data = [item for item in data if item['method'] == method]
                # 按hide_ratio排序
                method_data.sort(key=lambda x: x['hide_ratio'])
                correct_counts = [item.get('correct', 0) for item in method_data]
                total_counts = [item.get('total', 1) for item in method_data]
        ...
        # 不需要tight_layout，因为已经用GridSpec控制了布局
        ...
        # 保存时使用bbox_inches='tight'确保图例完整显示
        plt.savefig(self.output_dir / filename, dpi=300, bbox_inches='tight', pad_inches=0.3)
        plt.close()
        print(f"  Saved: {filename}")
